  ...
  <main class="h-screen overflow-hidden bg-black text-white relative select-none">
    ...
    <section class="relative h-[70vh] w-full flex items-center justify-center z-10 pointer-events-none">
      <div class="relative w-full max-w-7xl h-auto md:h-96 flex flex-col md:flex-row items-center md:justify-between px-6 md:px-16 mt-16 md:mt-60 py-8">
        ...
        <!-- Hero img sx: camera con parallax al mousemove, invert toggle on hover -->
        ...
        <!-- Hero img dx: computer con parallax opposto, invert toggle on hover -->
...
function scrambleText() { // Effetto scramble
  if (isScrambling) return
  isScrambling = true // evita overlap di animazioni
  let iterations = 0
...
// Parallax sulle immagini hero: segue il mouse con durate e offset diversi per depth
const handleMouseMove = (e) => {
  const { clientX, clientY } = e
  const { innerWidth, innerHeight } = window
  const xPos = (clientX / innerWidth) - 0.5 // da -0.5 a 0.5
...
onMounted(() => {
  scrambleText() // attiva text reveal al caricamento
  window.addEventListener('mousemove', handleMouseMove)
  // Animazioni iniziali delle immagini hero: fade in con scale e rotazioni
...
<style scoped>
.style-invert { filter: invert(1); } /* Immagini invertite di default */
.style-invert:hover { filter: invert(0); } /* Torna al colore naturale on hover */
.transition-filter { transition: filter 0.4s cubic-bezier(0.25, 1, 0.5, 1); } /* ease */
</style>