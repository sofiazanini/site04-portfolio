<template>
  <nav v-if="!open" class="fixed top-0 left-0 z-50 w-full flex justify-between items-center px-8 h-[10vh] pointer-events-none">
      ...
      <div class="h-[10vh] min-h-[10vh] border-t border-white/10 flex justify-between items-center text-[11px] uppercase tracking-wider text-zinc-500 w-full">
        <div class="flex gap-6">
          <a 
            ...
            @mouseenter="scrambleText($event, social)"
          >
            {{ social }}
          </a>
        </div>
        ...
        <div>
          <a 
            href="mailto:info@sofiazanini.com" 
            class="hover:text-white transition font-sans lowercase text-[12px] py-2 inline-block"
            @mouseenter="scrambleText($event, 'info@sofiazanini.com')"
          >
            info@sofiazanini.com
          </a>
        </div>
...
// Mappa dinamica per gestire le animazioni attive su qualsiasi elemento del DOM
const activeTweens = new Map()

// FUNZIONE DI SCRAMBLE UNIVERSALE PER TUTTI I LINK DEL SITO
const scrambleText = (e, originalText) => {
  // Troviamo l'elemento di testo effettivo (gestisce sia lo span interno che il tag a/button diretto)
  const el = e.target.querySelector('span') || e.target
  if (!el) return

  // Usiamo l'elemento stesso come chiave univoca della mappa per evitare sovrapposizioni
